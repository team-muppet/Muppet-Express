...
public:
	...
	MuppetExpress& MapGet(const std::string_view& path, MuppetHandler handler)
	{
		registerHandler(http::verb::get, path, handler);
		return *this;
	}

	MuppetExpress& MapPost(const std::string_view path, MuppetHandler handler)
	{
		registerHandler(http::verb::post, path, handler);
		return *this;
	}

	MuppetExpress& MapPut(const std::string_view path, MuppetHandler handler)
	{
		registerHandler(http::verb::put, path, handler);
		return *this;
	}

	MuppetExpress& MapDelete(const std::string_view path, MuppetHandler handler)
	{
